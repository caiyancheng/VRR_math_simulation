...
def Calculte_JOD_from_persistence(persistence):
...
if __name__ == "__main__":
    ...
    for p in persistence_range:
        ...
        JOD_sustained_max, JOD_transient_max, JOD_sustained_sum, \
            JOD_transient_sum, JOD_sustained_norm, JOD_transient_norm = Calculte_JOD_from_persistence(persistence=p)
    ...
    plt.subplot(3, 1, 1)
    plt.plot(p_list, JOD_sustained_max_list, label='Sustained')
    plt.plot(p_list, JOD_transient_max_list, label='Transient')
    # plt.xlabel('persistence')
    plt.ylabel('JOD Max')
    plt.legend()

    plt.subplot(3, 1, 2)
    plt.plot(p_list, JOD_sustained_sum_list, label='Sustained')
    plt.plot(p_list, JOD_transient_sum_list, label='Transient')
    # plt.xlabel('persistence')
    plt.ylabel('JOD Sum')
    plt.legend()

    plt.subplot(3, 1, 3)
    plt.plot(p_list, JOD_sustained_norm_list, label='Sustained')
    plt.plot(p_list, JOD_transient_norm_list, label='Transient')
    plt.xlabel('persistence')
    plt.ylabel('JOD Norm')
    plt.legend()
    plt.show()